...
**ProjectAlpha** 是 AI 训练营 **week1** 的实战项目，定位为面向中小型团队的轻量级项目管理工具。核心能力是对 **Ticket**（任务 / 问题 / 需求 / 待办事项）进行：
...
> **本项目目前处于「设计阶段」**，已完成需求分析、技术方案设计与阶段 0 项目启动准备。

| 阶段 | 状态 |
|------|------|
| 📝 需求与设计文档 | ✅ 已完成 |
| 🗺️ 实现计划 | ✅ 已完成 |
| 🛠️ 阶段 0 - 项目启动与环境准备 | ✅ 已完成 |
| 💻 阶段 1~3 后端实现 | ⏳ 待开始 |
| 🎨 阶段 4~6 前端实现 | ⏳ 待开始 |
| 🔗 阶段 7 前后端联调 | ⏳ 待开始 |
| 🧪 阶段 8 测试 | ⏳ 待开始 |
...
> ⚠️ 以下命令为项目实现后的目标使用方式。当前阶段 0 已完成，代码实现尚未开始。
...
本地已运行 MySQL 8.0（凭据：`root / 1qa2ws3ed`，端口 3306），并已创建 `project_alpha` 与 `project_alpha_test` 两个数据库。
...
"/c/Program Files/MySQL/MySQL Workbench 8.0/mysql.exe" \
  -h 127.0.0.1 -P 3306 -uroot -p1qa2ws3ed < database/init.sql
```

验证：

```bash
mysql -h 127.0.0.1 -P 3306 -uroot -p1qa2ws3ed \
  -e "SHOW DATABASES LIKE 'project_alpha%';"
```
